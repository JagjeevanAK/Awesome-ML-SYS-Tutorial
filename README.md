...
- [Pending Review] [slime FSDP Setup Guide](rlhf/slime/fsdp/release_log/setup_fsdp_en.md): Records how to test FSDP on slime, including H-cards and B-cards, and both Colocate and Disaggregated placement methods.
- [Pending Review] [Chunked Parallel Computation of GAE in PPO (slime Implementation)](rlhf/slime/batch-GAE/ppo-gae-chunk-en.md): Rewrites the standard backward recurrence of GAE into chunk-based parallel prefix scanning, significantly mitigating the GAE computation bottleneck in long sequence scenarios, achieving about $100\times–300\times$ acceleration in slime. Also available on [Zhihu](https://zhuanlan.zhihu.com/p/1975237289425798560).
...
- [Pending Review] [Deep Thoughts on RL Systems: Megatron](rlhf/sys-design/readme-3-en.md): Brief analysis of Megatron's basic features, focusing on its use in the RL framework.
- [Extending the OpenRLHF Inference Engine](rlhf/OpenRLHF/develop-log-en.md): Development notes on integrating SGLang into OpenRLHF. The entire process was very painful, and there's still an nccl hang error that a DeepSpeed core contributor is currently fixing.
...
- [Pending Review] [Light-DuoAttention: Achieves efficient Long-Context Inference using CuTeDSL and runs within SGLang](sglang/light-duoattention/light-duoattention-en.md): Introduction DuoAttention, a solution designed for Long-Context Inference, implemented with CuteDSL and supported in SGLang. Also available on [Zhihu](https://zhuanlan.zhihu.com/p/1984978732436431358).
...
- [Pending Review] [In-Depth Understanding of verl Source Code (Make Experience)](rlhf/verl/multi-turn/code-walk-through/readme-3-en.md): Analysis of the logic for the make experience part in verl.
- [AgentLoop Source Code Analysis](rlhf/verl/multi-turn/code-walk-through/readme-6-en.md): Analysis of the multi-turn RL implementation based on AgentLoop in verl.
...
- [Pending Review] [DAPO Dynamic Filtering Implementation and Batch Size Analysis](rlhf/verl/multi-turn/code-walk-through/dapo_en.md): Exploring how to achieve higher parallelism by padding prompts to a smaller batch size.
...
- [SGLang, verl, OpenBMB, and Tsinghua University Team Jointly Open Source: First Support for Multi-Turn Interaction and Tool Calling in Mainstream RLHF Frameworks](rlhf/verl/multi-turn/release_log/verl-multiturn-rollout-Release.md): First support for multi-turn interaction and tool calling in mainstream RLHF frameworks. Also available on [Zhihu](https://zhuanlan.zhihu.com/p/1906007821889283171).
- [Search-R1 & veRL-SGLang: Train LLMs with Multi-Turn RL to Reason and Call a Search Engine](rlhf/verl/multi-turn/tool_examples/verl-multiturn-searchR1-like.md): Integrating the Search-R1 framework into the verl-sglang ecosystem. Also available on [Zhihu](https://zhuanlan.zhihu.com/p/1912156329751081620).
- [SGLang-veRL Server: From Engine to Server, We Need More Flexible RLHF Rollout Interfaces](rlhf/verl/server-based/veRL-server-based-rollout-en.md): To implement more complex RLHF systems, we are gradually replacing the rollout engine in veRL with a rollout server. Also available on [Zhihu: SGLang-veRL Server](https://zhuanlan.zhihu.com/p/1890631652486665464).
- [HybridFlow veRL Original Paper Analysis](rlhf/verl/readme_en.md): Principles and implementation of SGLang's hybrid engine. Also available on [Zhihu: HybridFlow veRL Original Paper Analysis](https://zhuanlan.zhihu.com/p/24682036412).
...
- [Brief Analysis of the Computational Flow of Post-Training Systems Represented by OpenRLHF](rlhf/OpenRLHF/readme_en.md): Further complement to Ms. Mengyuan's article. The Github native rendering is terrible; you might as well look at [Zhihu](https://zhuanlan.zhihu.com/p/16370000391).


### Algorithms and Theory

- [Kimi K1.5: Successful Practice of Long Context RL](rlhf/partial-rollout/readme_en.md): Industrial implementation of Long Context RLHF. I have always liked the technical reports from the Kimi team. Also available on [Zhihu: Kimi K1.5: Successful Practice of Long Context RL](https://zhuanlan.zhihu.com/p/1894282607325344277).
...
- [Walk Through SGLang Scheduler](sglang/sglang-scheduler/readme.md)
...
- [Pending Review] [SGLang Multimodal Request Lifecycle: A Deep Architectural Analysis with Qwen2.5-VL as an Example](sglang/code-walk-through/multimodal_request_lifecycle_en.md): Provides a detailed analysis of the multimodal request processing flow within the SGLang framework, using Qwen2.5-VL as a reference model.
...
- [Pending Review] [Zero-Overhead Batch Scheduler](sglang/zero-overhead-scheduler/zero-overhead-batch-scheduler-en.md): Introduces the zero-overhead batch scheduler, which solves the GPU Bubble problem caused by serial execution of CPU scheduling and GPU computation in traditional inference systems.
- [Pending Review] [Data Parallelism Attention](sglang/dp-attention/readme_en.md): Detailed introduction to the principles and implementation of DP Attention, specifically for models like DeepSeek that use MLA and only have one KV head, to avoid KV cache duplication caused by tensor parallelism.
...
- [Constraint Decoding: Concepts, Methods, and Optimization](sglang/constraint-decoding/readme_en.md): Also available on [Zhihu: Understanding Constraint Decoding: Concepts, Methods, and Optimization in one article](https://zhuanlan.zhihu.com/p/18336995950).
- [Pending Review] [Online Update Weights](sglang/online-update-weights/readme_en.md): Introduction to the implementation of the `online_update_weights` interface in SGLang. Unlike `update_weights` which reads weights from the disk, this interface broadcasts new weights directly from the training engine via NCCL.
- [Pending Review] [SGLang Verl Engine Optimization Analysis](sglang/sglang-verl-engine/readme_en.md): Analysis of optimizations in the SGLang verl engine, including the implementation of interfaces like `update_weights_from_tensor`.
- [Latency Accelerate For Weight Updates](sglang/latency-accelerate-for-weight-updates/readme.md)
...
- [Understanding Special Tokens and Chat Templates in One Article](transformers/special_tokens/special_tokens_en.md): Also recorded on Zhihu [Understanding Special Tokens and Chat Templates in One Article](https://zhuanlan.zhihu.com/p/17052593700).
...
- [Pending Review] [Implementing Tensor Parallelism From Scratch](torch/tensor-parallelism/readme_en.md): Implementation and practice of Tensor Parallelism.
- [Pending Review] [Expert Parallelism](rlhf/sys-design/readme-4-en.md)
- [NCCL and NVIDIA TOPO](torch/nccl/readme_en.md): Introduction to NCCL and NVIDIA GPU detection. Also available on [NCCL and NVIDIA TOPO](https://zhuanlan.zhihu.com/p/6160835906).
...
- [PyTorch Distributed](torch/torch-distributed/readme_en.md): Communication practice with `torch.distributed`, details on GIL and `all_reduce`. This part is also available on [Zhihu: PyTorch Communication Practice](https://zhuanlan.zhihu.com/p/5853094319).
- [[Original][In-Depth][PyTorch] DDP Series Part 1: Introductory Tutorial](https://zhuanlan.zhihu.com/p/178402798): Although I didn't fully grasp the DDP content, I used this to learn about GIL and ring all reduce. This step is recorded in the [Postscript of torch-distributed](torch/torch-distributed/readme_en.md#gil).
- [Detailed Explanation of nvidia-smi Command and Some Advanced Tips](https://www.yourmetaverse.cn/deep_learning/199/): Mainly about network topology; my local results are recorded in the [NCCL section](torch/nccl/readme_en.md#nvlink-inspection).
...
- [Setting up a Clean Development Environment](engineer/uv/readme_en.md): Setting up a clean development environment. Also available on [Zhihu: Setting up a Clean Development Environment](https://zhuanlan.zhihu.com/p/23440683394).
